...
list_targets() {
    ...
    echo ""
    echo "Combined (all-vX.Y runs modules + inventory + regression if available):"
    echo "  all-v4.0, all-v4.1, all-v4.2, all-v4.3, all-v4.4, all-v4.5"
}
...
run_test() {
    ...
    # Navigate to the installed collection
...
<<<<<<< HEAD
    ...
    if [ -n "${NETBOX_TOKEN:-}" ]; then
        ...
        # For inventory tests, create runme_config file with NETBOX_TOKEN
        # This is the only way to pass environment variables to inventory runme.sh tests
        if [[ "$target" == inventory-* ]]; then
            ...
            echo "Creating $config_file with NETBOX_TOKEN"
            # Extract version from target name (e.g., inventory-v4.5 -> v4.5)
            local version="${target#inventory-}"
            cat > "$config_file" << EOF
# Generated by integration-test.sh
...
run_all_tests() {
    ...
    # Check if inventory tests exist for this version
    if [ -d "$REPO_DIR/tests/integration/targets/inventory-$version" ]; then
        targets+=("inventory-$version")
    fi

    # Check if regression tests exist for this version
...
>>>>>>> 68d4b64 (Add local testing infrastructure for inventory and regression tests)
    ...
    echo ""
    echo "================================"
    echo "All tests completed for $version"
    echo "================================"
}
